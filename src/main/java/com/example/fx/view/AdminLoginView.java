...
public class AdminLoginView extends BasicView {
    ...
    private final Button loginButton = new Button("管理员登录");
    ...
    public AdminLoginView() {
        ...
        loginButton.setOnAction(event -> {
            if (checkUserInfo()) {
                ...
                if (AdminDao.findAdminByNo(usernameField.getText()) == null) {
                    showDialog("错误！管理员不存在", Alert.AlertType.ERROR);
                }
                // System.out.println(teacher);
                if (admin.getPassword().equals(MD5Util.encrypt(passwordField.getText()))) {
                    clearAll();

                    MainView mainView = (MainView) ViewFactory.createView(ViewType.MAIN);
                    mainView.setAdmin(admin);
                    mainView.switchToAdminLogin();

                    showDialog("登录成功", Alert.AlertType.INFORMATION);
                    ViewFactory.close(ViewType.ADMIN);
                    return;
                }
    ...
    private boolean checkUserInfo() {
        ...
        if (!Pattern.matches(accountRegExp, account) || !Pattern.matches(passwordReExp, password)) {
            if (!Pattern.matches(accountRegExp, account)) {
                showDialog("！错误,账号是5位", Alert.AlertType.ERROR);
            }
